...
export default function CategoriaAdmin() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            await apiClient.post("/categorias", { NomeCategoria: form.nomeCategoria });
            handleMessage()
            modalFecharCadastro()
                                ...
                                {categorias.map((cat) => (
                                    <tr className="hover:bg-gray-200 h-8 border-b-[1px] border-gray-200" key={cat.codigoCategoria}>
                                        ...
                                        <td className="pl-2 flex gap-2 justify-center">
                                            ...
                                            <button onClick={() => { modalAbrirEditar(); setFormUpdate({ Id: cat.codigoCategoria, NomeCategoria: cat.nomeCategoria }) }} className="text-blue-500 cursor-pointer hover:scale-[106%]"><FontAwesomeIcon icon={faEdit} /></button>

                                            <button onClick={() => { modalAbrirExcluir(); setId(cat.codigoCategoria) }} className="text-red-500 cursor-pointer hover:scale-[106%]"><FontAwesomeIcon icon={faTrash} /></button>
                                        </td>
                                    </tr>
                                ))}
